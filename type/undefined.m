...
let type_undefined_check = func (t : *Type) -> Unit {
  if type_is_basic(t) {
  } else if type_is_pointer(t) {
    if t.pointer.to.kind == TypeUndefined {
      // it's ok.
    } else {
      type_undefined_check(t.pointer.to)
    }
  } else if type_is_array(t) {
    type_undefined_check(t.array.of)
  } else if type_is_function(t) {
    type_undefined_check(t.function.to)
    ...
    error("type_undefined_check undefined type", t.ti)
  }
}
